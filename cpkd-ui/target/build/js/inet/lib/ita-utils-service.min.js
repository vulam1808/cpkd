...
"":a.intl;if(Object.keys(a).length==1)if(a.hasOwnProperty("dateSubmit"))return false;if(!iNet.isEmpty(a)&&JSON.stringify(a)!="{}"&&!a.hasOwnProperty("errors")&&b!="ERROR!"&&iNet.isEmpty(c))return true;return false},getRequests:function(){var a=location.search.substring(1,location.search.length).split("&"),b={},c,g;for(g=0;g<a.length;g+=1){c=a[g].split("=");b[decodeURIComponent(c[0])]=decodeURIComponent(c[1])}return b},getArrayKey:function(a,b){var c=b||"id",g=[];$.each(a||[],function(d,f){g.push((f[c]||
"").toString())});return g},getIndexOfArray:function(a,b,c){var g=c||"id",d=(a||"").toString(),f=[];$.each(b||[],function(i,h){f.push((h[g]||"").toString())});var e=f.indexOf(d);return e},getCurrentDate:function(a){var b=a||"d/m/Y";return(new Date).format(b)},getFirstDate:function(a){var b=new Date,c=a||"d/m/Y";return(new Date(b.getFullYear(),b.getMonth(),1)).format(c)},syncProcedure:function(){$.postJSON(iNet.getUrl("onegate/firmprocedure/synch"),{},function(a){console.log("CommonService syncProcedure",
{result:a})})},getTotalPage:function(a,b){var c=parseInt(parseInt(a/b)+parseInt(a%b>0?1:0));if(isNaN(c))return 0;return c},getNumber:function(a,b){a=parseInt(a);if(isNaN(a))return b;return a},getPercentCurrentDate:function(a,b){var c=0,g=(new Date).getTime(),d=b-a,f=g-a;if(d>0&&f>0){c=f*100/d;c=this.getNumber(c,0)>=100?100:this.getNumber(c,0)<=0?0:c.toFixed(0)}return c}};
