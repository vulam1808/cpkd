// #PACKAGE: xgate-ui-report-citizen-service
// #MODULE: XGateRecordService
$(function () {
    iNet.ns("iNet.ui.report", "iNet.ui.report.Citizen");

    iNet.ui.report.Citizen = function () {
        this.id = 'report-citizen-div';
        var self = this;

        var resource = {
            citizen: iNet.resources.xgate.report.citizen,
        ...
        var $toolbar = {
            SEARCH: $('#report-citizen-search-btn'),
            PRINT: $('#report-citizen-print-btn'),
            VIEW: $('#report-citizen-view-btn'),
            EXPORT: $('#report-citizen-export-btn')
        };
        ...
        var $form = {
            fromDate: $('#report-citizen-from'),
            toDate: $('#report-citizen-to'),
            procedure: $('#report-citizen-procedure'),
            status: $('#report-citizen-status'),

            report: $('#report-citizen-content')
        };
        ...
        $form.procedure = FormService.createSelect("report-citizen-procedure", [], "id", 1, true, false);
        $.postJSON(url.procedureList, {}, function(result){
            ...
            if (CommonService.isSuccess(__result)){
                ...
                $form.procedure = FormService.createSelect("report-citizen-procedure", __list, "id", 1, true, false);
            }
        });

        var __statusList = [
            {id: "", code: "", name: resource.citizen.statusAll},
            {id: "INPROCESS", code: "INPROCESS", name: resource.citizen.statusINPROCESS},
            {id: "COMPLETED", code: "COMPLETED", name: resource.citizen.statusCOMPLETED},
            {id: "REJECTED", code: "REJECTED", name: resource.citizen.statusREJECTED},
            {id: "PUBLISHED", code: "PUBLISHED", name: resource.citizen.statusPUBLISHED}
        ];
        $form.status = FormService.createSelect("report-citizen-status", __statusList, "id", 1, true, false);
            ...
            columns: [
                {
                    ...
                    label : resource.citizen.procedureName,
                    sortable : true,
                    disabled: true
                },
                {
                    property : 'subject',
                    label : resource.citizen.subject,
                    sortable : true,
                    disabled: true
                },
                {
                    property : 'fullName',
                    label : resource.citizen.fullName,
                    sortable : true,
                    disabled: true
                },
                {
                    property : 'status',
                    label : resource.citizen.status,
                    sortable : true,
                    disabled: true
                },
                {
                    property : 'created',
                    label : resource.citizen.created,
                    sortable : true,
                    disabled: true
                },
                {
                    property : 'appointment',
                    label : resource.citizen.appointment,
                    sortable : true,
                    disabled: true
                },
                {
                    property : 'completed',
                    label : resource.citizen.completed,
                    sortable : true,
                    disabled: true
                }
            ],
        ...
        var grid = new iNet.ui.grid.Grid({
            id: 'report-citizen-grid',
            url: url.view,
            params: {},
            convertData: function (data) {
                var __data = data || {};
                $.each(__data.items || [], function(i, item){
                    item.status = resource.citizen["status"+item.status] || "";
        ...
        iNet.ui.report.Citizen.superclass.constructor.call(this);
    };
    iNet.extend(iNet.ui.report.Citizen, iNet.ui.app.widget);

    new iNet.ui.report.Citizen().show();
});
